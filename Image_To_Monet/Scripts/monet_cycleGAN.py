...
Original file is located at
    https://colab.research.google.com/drive/1MDOEpXeVPQ7MyWJ-BWsY7XAUqXwWq7Jf

The final task we undertook in our exploration of GANs was to implement
a Cycle GAN on Unpaired Data in an attempt to convert images into Monet Style 
paintings.
Kaggle competition: https://www.kaggle.com/competitions/gan-getting-started/overview

We started off with implementing a Cycle GAN to convert horses to zebras from the
Tensorflow website: https://www.tensorflow.org/tutorials/generative/cyclegan

Then, we modified it according to the notebook suggested in the compeition overview:
https://www.kaggle.com/code/amyjang/monet-cyclegan-tutorial/notebook
and experimented with different batch sizes and number of epochs. 
...
CHANNELS = 3
EPOCHS = 40
BATCH_SIZE = 1 #as we have only 300 Monet paintings to train on
OUTPUT_CHANNELS = 3

###################################################################################
#PREPARING THE DATA

monet_data_dir = tf.io.gfile.glob('/Users/sumeghasinghania/Desktop/gan-getting-started/monet_tfrec/*.tfrec')
photo_data_dir = tf.io.gfile.glob('/Users/sumeghasinghania/Desktop/gan-getting-started/photo_tfrec/*.tfrec')
def count_data_items(filenames):
    n = [int(re.compile(r"-([0-9]*)\.").search(filename).group(1)) for filename in filenames]
    return np.sum(n)

n_monet_samples = count_data_items(monet_data_dir)
n_photo_samples = count_data_items(photo_data_dir)

print(f'Monet TFRecord files: {len(monet_data_dir)}')
print(f'Monet image files: {n_monet_samples}')
print(f'Photo TFRecord files: {len(photo_data_dir)}')
...
#function to read the tf records 
...
#function to load datasets from tf records
def load_dataset(filenames):
    dataset = tf.data.TFRecordDataset(filenames)
    dataset = dataset.map(read_tfrecord, num_parallel_calls=AUTO)
    return dataset

#function to generate the gan dataset
...
#function to display training samples
...
#function to display generated monet paintings
...
display_samples(load_dataset(monet_data_dir).batch(1), 4, 6)

#display picture samples
display_samples(load_dataset(photo_data_dir).batch(1), 4, 6)

                
###################################################################################
# DEFINING UPSAMPLING AND DOWNSAMPLING FUNCTIONS
...
history = gan_model.fit(get_gan_dataset(monet_data_dir, photo_data_dir, batch_size=BATCH_SIZE), 
                        steps_per_epoch=(n_monet_samples//BATCH_SIZE),
                        epochs=EPOCHS,
                        verbose=2).history

###################################################################################
#RUNNING THE MODEL ON PHOTO DATABSASE AND GENERATING MONET STYLE PAINTINGS

display_generated_samples(load_dataset(photo_data_dir).batch(1), monet_data_dir, 8)
...
predict_and_save(load_dataset(photo_data_dir).batch(1), monet_generator, '../images/')
